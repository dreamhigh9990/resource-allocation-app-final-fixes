...
import jobTitleExporterFields from 'src/app/job-title/list/job-title-exporter-fields';
...
export class JobTitleListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await JobTitleApi.list(
        filter,
        this.orderBy,
        null,
        null,
