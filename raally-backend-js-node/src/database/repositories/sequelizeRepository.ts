...
export default class SequelizeRepository {
  ...
  static async getUsageByHoursData(options: IRepositoryOptions) {
    ...
    const assignedHours = await options.database.sequelize.query(
      // Bug #16: [Dashboard] Completing assignment is not reflected in the dashboard charts.
      "SELECT SUM(hoursPerWeek) as assignedHours FROM assignments WHERE (endDate IS NULL OR endDate > DATE('now')) AND tenantId = '" + tenant.id + "'",
      {
        raw: true,
        type: QueryTypes.SELECT
      }
  ...
  static async getNumberOfPeoplePerRole(options: IRepositoryOptions) {
    ...
    const people = await options.database.sequelize.query(
      // Bug #15: [Dashboard] People chart shows incorrect data
      "SELECT personId, title, MAX(effectiveDate) as date FROM jobTitles WHERE jobTitles.deletedAt IS NULL AND tenantId = '" + tenant.id + "' GROUP BY personId",
      {
        raw: true,
        type: QueryTypes.SELECT
      });
  ...
  static async getUsageByPeopleData(options: IRepositoryOptions) {
    ...
    const availability = await options.database.sequelize.query(
      "SELECT fullName, availability.workAvailability, assignedHours.assignedHours, jobTitles.title FROM people \
        ...
        LEFT JOIN (SELECT personId, SUM(hoursPerWeek) as assignedHours FROM assignments WHERE (endDate IS NULL OR endDate > DATE('now')) AND tenantId = '" + tenant.id + "' GROUP BY personId) AS assignedHours ON people.Id = assignedHours.personId \
